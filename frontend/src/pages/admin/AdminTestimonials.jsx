  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          <h1 className="font-display text-[32px] text-white font-semibold tracking-tight">Testimonials</h1>
          <p className="text-zinc-400 text-[14px] mt-1">Manage the social proof on your landing page.</p>
        </div>
      ...
      {adding && (
        <form onSubmit={submit} className="glass-strong rounded-2xl p-5 grid sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2">
            <label className="text-[12px] text-zinc-400">Quote</label>
            <textarea value={form.quote} onChange={(e) => setForm({ ...form, quote: e.target.value })} rows={3} required
              className="mt-2 w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2.5 text-[14px] text-white focus:outline-none focus:border-blue-400/50 resize-none" />
          </div>
          <div>
            <label className="text-[12px] text-zinc-400">Rating</label>
            <select value={form.rating} onChange={(e) => setForm({ ...form, rating: parseInt(e.target.value, 10) })}
              className="mt-2 w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2.5 text-[14px] text-white focus:outline-none focus:border-blue-400/50">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {items.map((t) => (
            <div key={t.id} className="glass rounded-2xl p-5 relative">
              <button onClick={() => del(t.id)} className="absolute top-3 right-3 h-7 w-7 rounded-md text-rose-300 hover:bg-rose-500/10 flex items-center justify-center">
                <Trash2 className="h-3.5 w-3.5" />
              </button>
              <div className="text-[12px] text-zinc-400">{"★".repeat(t.rating)}</div>
              <p className="mt-2 text-[14px] text-zinc-100">“{t.quote}”</p>
              <div className="mt-3 text-[13px] text-white font-medium">{t.name}</div>
